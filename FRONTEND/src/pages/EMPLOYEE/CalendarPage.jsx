...
const CalendarPage = () => {
      ...
      try {
        ...
        (res.data || []).forEach((task) => {
          ...
          const color = task.status === "completed" ? "#10b981"
            : task.priority === "urgent" || task.priority === "high" ? "#f97316" : "#4f46e5";
          grouped[key] = [...(grouped[key] || []), { title: task.title, color, source: "task" }];
        });
  ...
  const closeModal = () => {
    setShowModal(false); setEvTitle(""); setEvDate(""); setEvColor(COLORS[0].value);
  };
  ...
  const handleSave = () => {
    ...
    const key = `${y}-${m}-${d}`;
    setEvents((prev) => ({ ...prev, [key]: [...(prev[key] || []), { title: evTitle.trim(), color: evColor }] }));
    closeModal();
  };
            ...
            {cells.map((day, i) => {
              ...
              return (
                ...
                >
                  {day && (
                    <>
                      <span className={`text-[13px] font-semibold inline-flex w-7 h-7 items-center justify-center rounded-full ${isToday ? "bg-[#4f46e5] text-white" : "text-[#111]"}`}>
                        {day}
                      </span>
                      <div className="mt-1 flex flex-col gap-0.5">
                        {dayEvents.map((ev, j) => (
                          <div key={j} className="text-[10px] font-semibold px-1.5 py-0.5 rounded-[4px] text-white truncate" style={{ background: ev.color }}>
                            {ev.title}
                          </div>
                        ))}
                      </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={(e) => e.target === e.currentTarget && closeModal()}>
          <div className="bg-white rounded-[16px] p-6 w-[340px] shadow-2xl">
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={closeModal} className="text-[#aaa] hover:text-[#555] transition bg-transparent border-none cursor-pointer"><X size={18} /></button>
            </div>
            <div className="mb-4">
              <label className="block text-[12px] font-semibold text-[#555] mb-1.5">Event Title</label>
              <input type="text" value={evTitle} onChange={(e) => setEvTitle(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSave()}
              ...
              <div className="flex gap-2">
                {COLORS.map((c) => (
                  <button key={c.value} onClick={() => setEvColor(c.value)}
                    ...
                    style={{ background: c.value, outline: evColor === c.value ? `2px solid ${c.value}` : "none", outlineOffset: "2px", transform: evColor === c.value ? "scale(1.2)" : "scale(1)" }}
                    title={c.label} />
                ))}
              </div>
            </div>
            <div className="flex gap-2">
              <button onClick={closeModal} className="flex-1 py-2.5 rounded-[8px] bg-[#f5f5f5] text-[#555] text-[13px] font-semibold border-none cursor-pointer hover:bg-[#eee] transition">Cancel</button>
              <button onClick={handleSave} className="flex-1 py-2.5 rounded-[8px] bg-[#4f46e5] text-white text-[13px] font-semibold border-none cursor-pointer hover:bg-[#4338ca] transition">Add Event</button>
            </div>
          </div>
        </div>
      )}
