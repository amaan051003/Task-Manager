...
const Login = () => {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            // ← This automatically shows whatever message backend sends
    ...
    useEffect(() => {
        const submitFromEnter = (e) => {
            if (e.key !== "Enter" || loading || roleOpen) return;
            e.preventDefault();
            handleLogin();
        };

        window.addEventListener("keydown", submitFromEnter);
        return () => window.removeEventListener("keydown", submitFromEnter);
    }, [email, password, selectedRole, loading, roleOpen]);
                    ...
                    <motion.form variants={fadeUp} onSubmit={handleLogin}>
                        ...
                        {error && (
                            <motion.div
                                ...
                                animate={{ opacity: 1, y: 0 }}
                                className="mt-3 px-4 py-2.5 rounded-xl bg-red-50 border border-red-200 text-red-600 text-xs font-medium"
                            >
                                {error}
                            </motion.div>
                        )}
                        <motion.button
                            type="submit"
                            whileHover={{ scale: loading ? 1 : 1.02, background: "#4338ca" }}
                            whileTap={{ scale: loading ? 1 : 0.98 }}
                            disabled={loading}
                            className="mt-5 w-full py-3.5 rounded-xl bg-indigo-600 text-white text-sm font-bold border-none cursor-pointer tracking-wide disabled:opacity-60 disabled:cursor-not-allowed"
                        >
                            {loading ? (
